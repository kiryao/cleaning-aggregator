...
class MapperRespondTest {

    @Test
    fun fromTransport() {
        val req = OrderRespondRequest(
            debug = OrderDebug(
                mode = OrderRequestDebugMode.STUB,
                stub = OrderRequestDebugStubs.SUCCESS
            ),
            order = OrderRespondObject(
                id = "1234",
                lock = "456",
                executorId = "789"
            )
        ...
        assertEquals("789", context.orderRequest.executorId.asString())
    }
    ...
    fun toTransport() {
        ...
        val res = context.toTransport() as OrderRespondResponse
        ...
        assertEquals("789", res.order?.executorId)
